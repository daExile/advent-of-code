...
local function nearby_numbers(row, col, part)
    local sum, count, ratio, lastnum = 0, 0, 1 -- assume they're all different or at least ones that near each other are
    for i = row - 1, row + 1 do
        for j = col - 1, col + 1 do
            if tonumber(grid[i][j]) then
                x = get_number(i, j)
                if not lastnum or x ~= lastnum then
                    sum = sum + x; count = count + 1; ratio = ratio * x; lastnum = x end
            end
        end
    end
